...
public class UI : MonoBehaviour {
    ...
    void OnGUI()
    {
        ...
        {    
            if (GUI.Button(new Rect(470, 100, 80, 50), "PriestLOn"))
            {
                userInterface.priestLeftOn();
            }
            if (GUI.Button(new Rect(555, 100, 80, 50), "PriestROn"))
            {
                userInterface.priestRightOn();
            }
            if (GUI.Button(new Rect(470, 150, 80, 50), "DevilLOn"))
            {
                userInterface.devilLeftOn();
            }
            if (GUI.Button(new Rect(555, 150, 80, 50), "DevilROn"))
            {
                userInterface.devilRightOn();
            }
            if (GUI.Button(new Rect(470, 200, 80, 50), "GetOff"))
            {
                userInterface.getOffBoat();
            }
            if (GUI.Button(new Rect(555, 200, 80, 50), "MOVE"))
            {
