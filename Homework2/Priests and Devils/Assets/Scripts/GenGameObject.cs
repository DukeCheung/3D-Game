...
public class GenGameObject : MonoBehaviour {
    public float speed = 1.0f;//船只移动速度
    ...
    public void priestLeftOn()//牧师从左岸上船
    {
        if (!isBoatFull()&&priest_left.Count!=0&&dir.state==State.LEFT)
        {
    ...
    {
        if (!isBoatFull() && priest_right.Count != 0 && dir.state == State.RIGHT)
        {
    ...
    public void devilLeftOn()//魔鬼从左岸上船
    {
        if (!isBoatFull() && devil_left.Count != 0 && dir.state == State.LEFT)
        {
    ...
    {
        if (!isBoatFull() && devil_right.Count != 0 && dir.state == State.RIGHT)
        {
